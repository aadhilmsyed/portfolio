...
export default function QuickLoadingScreen() {
  const [isLoading, setIsLoading] = useState(true)
  const [startTransition, setStartTransition] = useState(false)

  useEffect(() => {
    // Reduced from 2000 to 500 (initial hover time)
    setTimeout(() => {
      setStartTransition(true)
    }, 500)

    // Reduced from 2800 to 1300 (total time = hover + transition)
    setTimeout(() => {
      setIsLoading(false)
    }, 1300)
  }, [])
  ...
  return (
    <AnimatePresence>
      {isLoading && (
        ...
        >
          {/* All animations remain at original speed */}
          <motion.div
          ...
          >
            {/* Cloud animations - keeping original speeds */}
            <motion.div
